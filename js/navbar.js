...
(function () {
  ...
  function init() {
    ...
    /* Show install button immediately on file:// so users can click it and learn how to serve the app */
    installBtn.style.display = location.protocol === 'file:' ? 'flex' : 'none';
    ...
    installBtn.onclick = async () => {
      ...
      /* file:// protocol — install is impossible without a server */
      if (location.protocol === 'file:') {
        if (window.showDashToast) showDashToast(
          '⚠ Serve over HTTP to enable install — see Install App button for details',
          'warning', 5000
        );
        return;
      }
      /* HTTPS but no prompt yet (Firefox, Safari, or not yet qualifying) */
      if (window.showDashToast) showDashToast(
